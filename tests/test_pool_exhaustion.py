...
def call_under(client: TestClient):
    """Hit a route that needs the database.

    The token is deliberately junk: authentication itself reads the database, so
    the failure happens at the first checkout either way, and a valid token would
    add a signup that needs the very pool this test has emptied.
    """
    return client.get("/conversations",
                      headers={"Authorization": "Bearer irrelevant"})


def test_a_full_pool_returns_503_not_500(client, pool_is_full):
    """The end-to-end claim: saturation is a 503 a client can act on."""
    response = call_under(client)
...
def test_the_busy_response_leaks_no_connection_string(client, pool_is_full):
    """The driver's message carries the DSN, so it stays in the log."""
    text_out = call_under(client).text
...
def test_the_busy_message_does_not_blame_postgres(client, pool_is_full):
